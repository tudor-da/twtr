...
test('create and look up user using ledger library', async () => {
  ...
  const user = {username: party, reviewers: {}};
...
const addReviewer = async (page: Page, userToFollow: string) => {
...
test('log in as three different users and start following each other', async () => {
  ...
  await addReviewer(page1, party2);
  ...
  expect(followingList1).toEqual([party2]);
   ...
   await addReviewer(page1, party3);
  ...
  await addReviewer(page2, party3);
